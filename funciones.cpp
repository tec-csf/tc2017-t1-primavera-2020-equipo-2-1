...
string if_condicion(vector<string> &fuente, int num_linea, stack<string>& loops_condiciones, stack<string>& loops_condiciones_end)
{
    ...
    line_no = fuente[num_linea].find('{') != string::npos ? num_linea + 1 : num_linea + 2;
    capas.push('{');

    while (!capas.empty())
    {
        poli_temp += analizar_complejidad_if(fuente[line_no], fuente, line_no, loops_condiciones, loops_condiciones_end);

        if(fuente[line_no].find('}') != string::npos)
        {
            capas.pop();
        }
        else if(fuente[line_no].find('{')!= string::npos)
        {
            capas.push('{');
            ++line_no;
        } else
    ...
    poli_ifs.push_back(poli_temp);

    while(atorado)
    {
        if(fuente[line_no].find("else") != string::npos)
        {
            if(fuente[line_no].find("if") == string::npos)
            { atorado = false; }
            line_no = fuente[line_no].find('{') != string::npos ? line_no + 1 : line_no + 2;
            poli_temp = "";
            capas.push('{');
          ...
          poli_ifs.push_back(poli_temp);// se guarda al vector
          if(!atorado)
          {
        ...
        else if(fuente[line_no+1].find("else") != string::npos)
        {
            ++line_no;
        } else
        {
